...
def main():
    """تابع اصلی - دقیقاً مثل اصلی اما با پولینگ"""
    try:
        # ساخت اپلیکیشن
        application = Application.builder().token(DEV_BOT_TOKEN).build()
        
        # اضافه کردن هندلرها - دقیقاً مثل اصلی
        ...
        # 🔹 فقط این خط فرق می‌کنه - پولینگ به جای وب‌هوک
        logger.info("🧪 بات توسعه اجرا شد (حالت پولینگ)...")
        logger.info("📱 به @saman_rahjou_test_bot پیام بدهید")
        logger.info("🎯 دقیقاً مثل نسخه اصلی کار می‌کند")
